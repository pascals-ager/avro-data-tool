...
lazy val commonSettings = Seq(
  ...
  avroScalaSource in Compile := new java.io.File(s"${baseDirectory.value}/${Vers.scalaADT}"),
  avroSourceDirectories in Compile += new java.io.File(s"${baseDirectory.value}/${Vers.resources}")
)
...
val runUnitTests = ReleaseStep(
  action = Command.process("testOnly * -- -l \"io.pascals.avro.schema.tags.IntegrationTest\"", _),
  enableCrossBuild = true
)

val runIntegrationTests = ReleaseStep(
  action = Command.process("testOnly * -- -n \"io.pascals.avro.schema.tags.IntegrationTest\"", _),
  enableCrossBuild = true
)
