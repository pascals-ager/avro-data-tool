...
class AvroDiffUtilsTest extends FunSuite with Matchers {
  ...
  val testSchemaOne: Schema = parserOne.parse(
    this.getClass
      .getResourceAsStream("/old/test_acquisition_traffic_scoring.avsc")
  )
  val testSchemaTwo: Schema = parserTwo.parse(
    this.getClass
      .getResourceAsStream("/new/test_acquisition_traffic_scoring.avsc")
  )
  ...
  val diff: Option[Schema] =
    AvroDiffUtils.diffAvroSchemas(testSchemaOne, testSchemaTwo)
  val diffClassTypeMeta = ClassTypeMeta(
    "test_acquisition_traffic_scoring",
    "test_acquisition_traffic_scoring",
    "test_acquisition_traffic_scoring",
    List(),
    List(
  ...
  test("ClassTypeMeta generation Test") {
    val classTypeMeta: TypeMeta = diff match {
      case Some(sc) =>
        AvroDiffUtils
          .typeMetaGenerator(sc, sc.getName)
          ._2 //Some(AvroDiffServiceImpl.genDiffClassTypeMeta(s))
      case None => NullType
    }

    classTypeMeta match {
      case cls: ClassTypeMeta =>
        val clsFields: Iterable[(String, TypeMeta)] = cls.fields.map { f =>
          (f.fieldName, f.fieldType)
        }
        val diffClsFields: Iterable[(String, TypeMeta)] =
          diffClassTypeMeta.fields.map { f =>
            (f.fieldName, f.fieldType)
          }
        clsFields.map { f =>
          f._1
        }.toSet shouldEqual diffClsFields.map { f =>
          f._1
        }.toSet
        clsFields.map { f =>
          f._2
