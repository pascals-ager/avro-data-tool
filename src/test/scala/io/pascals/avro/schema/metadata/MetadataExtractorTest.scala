...
class MetadataExtractorTest extends FunSuite with Matchers {
  ...
  case class Person( @column(name = "name") personName: String, @column(name = "age") personAge: Int )
  ...
  test("Extract Metadata Test") {
    ...
    personMeta.annotations.toList.head.name should equal("io.pascals.avro.schema.annotations.hive.hiveTable")
    ...
    val fieldAnnotationsBuffer: ListBuffer[AnnotationMeta] = ListBuffer[AnnotationMeta]()
    personMeta.fields.foreach {
      f => {
    ...
    val annotationsAttributeBuffer: ListBuffer[AnnotationAttribute] = ListBuffer[AnnotationAttribute]()
    fieldAnnotationsBuffer.foreach {
      annotationMeta => {
    ...
    annotationsBuffer.head should equal("io.pascals.avro.schema.annotations.hive.column")
    annotationsBuffer(1) should equal("io.pascals.avro.schema.annotations.hive.column")
