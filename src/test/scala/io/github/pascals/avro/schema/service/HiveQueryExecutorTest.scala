...
import io.github.pascals.avro.schema.service.HiveQueryExecutor._
import io.github.pascals.avro.schema.service.HiveModelGenerator.generateCreateDDL
import io.github.pascals.avro.schema.tags.IntegrationTest
...
class HiveQueryExecutorTest extends FunSuite with Matchers {
  ...
  val hiveStrategy: Strategy =
    Strategy.void.copy(after = unit, oops = unit, always = close)
  ...
  test("Alter test_acquisition_traffic_scoring Test", IntegrationTest) {
    ...
    execute match {
      ...
      case Some(Failure(throwable: Throwable)) =>
        fail(
          s"Hive Server returned error while executing ALTER Table statement - ${throwable.toString}"
        )
  ...
  test(
    "Create ReferencedPerson with simple executor service Test",
    IntegrationTest
  ) {
    for {
