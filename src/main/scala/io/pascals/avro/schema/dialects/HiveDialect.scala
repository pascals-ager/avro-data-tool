...
object HiveDialect extends Dialect {
  ...
  * */
  ...
  private def applyAnnotation( cf: ClassFieldMeta, c: ClassTypeMeta ): ClassFieldMeta = {
    cf.copy(
  ...
  private def applyAnnotation( typeMetaData: TypeMeta ): TypeMeta = typeMetaData match {
  ...
  override def getSpecificAnnotation( annotationName: String, hasAnnotations: HasAnnotations ): Option[AnnotationMeta] = {
    val annotation: Option[AnnotationMeta] = hasAnnotations.annotations.find(a => a.name == annotationName)
    annotation
  }
  ...
  * */
  ...
  def generateClassTypeExpression( classTypeMetaData: ClassTypeMeta, fieldNamesWithExpressions: Iterable[(String, String)] ): String =
  ...
  private def generateTypeExpression( typeMetaData: TypeMeta, level: Int ): String = typeMetaData match {
    case p: PrimitiveTypeMeta => generatePrimitiveTypeExpression(p)
    case c: SeqTypeMeta => generateArrayTypeExpression(generateTypeExpression(c.element))
    case m: MapTypeMeta => generateMapTypeExpression(generateTypeExpression(m.key), generateTypeExpression(m.value, level))
    case c: ClassTypeMeta => generateClassTypeExpression(c, c.fields.map(f => (f.fieldName, generateClassFieldExpression(f, level + 1))))
  }
  ...
  private val HiveTable: String = "io.pascals.avro.schema.annotations.hive.hiveTable"
  private val HiveExternalTable: String = "io.pascals.avro.schema.annotations.hive.hiveExternalTable"
  ...
  private val HivePartitionColumn: String = "io.pascals.avro.schema.annotations.hive.hivePartitionColumn"
  private val HiveBucketColumn: String = "io.pascals.avro.schema.annotations.hive.hiveBucket"
  private val HiveStoredAs: String = "io.pascals.avro.schema.annotations.hive.hiveStoredAs"
  private val HiveTblProps: String = "io.pascals.avro.schema.annotations.hive.hiveTableProperty"

  override def alterDataModel( classTypeMeta: ClassTypeMeta, fieldsExpressions: Iterable[String] ): String =
    alterTableExpression(classTypeMeta) +
      generateColumnsExpression(classTypeMeta, fieldsExpressions)


  override def generateDataModel( classTypeMeta: ClassTypeMeta, fieldsExpressions: Iterable[String] ): String =
  ...
  override def getAnnotatedFieldName( cf: ClassFieldMeta, c: ClassTypeMeta ): String = {
  ...
  private def hiveExternalTableLocation( classTypeMetaData: ClassTypeMeta ): Option[String] =
  ...
  private def generateClassFieldExpression( f: ClassFieldMeta, level: Int ): String = {
    val typeExpression = generateTypeExpression(f.fieldType)
    generateClassFieldExpression(f, typeExpression, level)
  }

  private def generateClassFieldExpression( f: ClassFieldMeta, typeExpression: String, level: Int ): String =
  ...
  override def createTableExpression( classTypeMetaData: ClassTypeMeta ): String =
    s"CREATE ${if (hiveExternalTableLocation(classTypeMetaData).isDefined) "EXTERNAL " else ""}TABLE IF NOT EXISTS ${classTypeMetaData.typeName}"


  private def convertToUnderscoreIfRequired( name: String, c: ClassTypeMeta ): String = {
    val underscoreAnnotation: Option[AnnotationMeta] = getSpecificAnnotation(Underscore, c)
  ...
  private def getPartitionFields( c: ClassTypeMeta ): Seq[ClassFieldMeta] = {
    val partitionFields = c.fields.filter(_.annotations.exists(_.name == HivePartitionColumn))
    var fieldOrder = 0
    partitionFields
      .map(f => {
  ...
  private def getBucketedFields( c: ClassTypeMeta ): Seq[(ClassFieldMeta, Int)] = {
    val bucketedFeilds: Iterable[ClassFieldMeta] = c.fields.filter(_.annotations.exists(_.name == HiveBucketColumn))
    bucketedFeilds.map(f => {
      val bucketColumn: AnnotationMeta = f.annotations.find(_.name == HiveBucketColumn).get
      val buckets = Try(bucketColumn.attributes.find(_.name == "buckets").get.value.toInt).getOrElse(1)
      (f, buckets)
    })
      .toSeq
  }
  ...
  private def getTblProperties(c: ClassTypeMeta):  Iterable[(String, String)] = {
    c.annotations.filter(_.name == HiveTblProps).map{
      props => {
        (props.attributes.find(_.name == "key").get.value, props.attributes.find(_.name == "value").get.value)
      }
    }
  }
