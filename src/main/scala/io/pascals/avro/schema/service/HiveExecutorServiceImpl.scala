...
object HiveExecutorServiceImpl extends QueryExecutorService {
  ...
  /**
    ...
    *
    */
  override val executeAlterHiveTable: ClassTypeMeta => Option[ConnectionIO[Int]] =
    (classTypeMeta: ClassTypeMeta) => for {
  ...
  /**
    ...
    */
  override def executeCreateHiveTable[T: ClassTag : universe.TypeTag]: Option[ConnectionIO[Int]] = for {
    createTableDDL <- generateCreateDDL[T]()
  } yield Update0(createTableDDL, None).run

  override val hiveExecutorService: String => Option[ConnectionIO[Int]] = ddl => Option(Update0(ddl, None).run)
