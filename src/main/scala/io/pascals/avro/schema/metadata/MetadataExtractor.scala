  ...
  * */
  def extractClassMeta[T: ru.TypeTag : ClassTag]( ): ClassTypeMeta = {
    ...
    tpe match {
      ...
      case other => throw new UnsupportedOperationException(s"Metadata for type $other is not supported")
    }
  }
  ...
  private def extractClassFields( tpe: ru.Type ): Iterable[ClassFieldMeta] = {
    val paramList: List[ru.Symbol] = tpe.typeSymbol.asClass.primaryConstructor.typeSignature.paramLists.head
    paramList.map {
      symbol =>
        ClassFieldMeta(
